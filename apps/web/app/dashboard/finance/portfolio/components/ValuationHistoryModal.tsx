...
export function ValuationHistoryModal({
  ...
  return (
    <Modal opened={!!investment} onClose={onClose} title={`Valuation history — ${investment?.name ?? ''}`} centered size="sm">
      {isLoading ? (
        <Center py="lg"><Loader size="sm" color="#4686FE" /></Center>
      ) : valuations.length === 0 ? (
        <Text size="sm" c="dimmed">No valuation history yet.</Text>
      ) : (
        <Stack gap={6}>
          {valuations.map((v) => (
            <Group key={v.id} justify="space-between" style={{ border: '1px solid #E2E8F0', borderRadius: 8, padding: '8px 12px' }}>
              ...
              <Text size="sm" fw={600}>{fmtCurrency(v.value, investment?.currency)}</Text>
            </Group>
          ))}
        </Stack>
      )}
