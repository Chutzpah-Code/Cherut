...
export function BalanceProjectionChart({ horizon }: { horizon: number }) {
  ...
  return (
    <Box>
      <Text style={{ fontSize: 15, fontWeight: 700 }}>Balance projection</Text>
      <Text style={{ fontSize: 12.5, color: '#64748B', margin: '4px 0 20px' }}>
        Next {horizon} days · cash accounts only, based on pending bills and recurring income
      </Text>
      ...
      <Group justify="space-between" mt={8}>
        <Text style={{ fontSize: 11.5, color: '#64748B' }}>{fmtShortDate(sampled[0].date)}</Text>
        <Text style={{ fontSize: 11.5, color: '#64748B' }}>{fmtShortDate(sampled[sampled.length - 1].date)}</Text>
      </Group>
      ...
      <Group gap={26} mt={18} pt={16} style={{ borderTop: '1px solid #EFF1F5' }} wrap="wrap">
        <Stack gap={3}>
          ...
          <Text style={{ fontSize: 15, fontWeight: 600 }}>
            {fmtCurrency(data.lowestPoint.total, displayCurrency)}{' '}
            <Text span style={{ fontSize: 12.5, fontWeight: 500, color: isRisk ? '#B91C1C' : '#64748B' }}>
              · {fmtShortDate(data.lowestPoint.date)}{isRisk ? ' · at risk' : ''}
            </Text>
          </Text>
        </Stack>
        <Stack gap={3}>
          ...
          <Text style={{ fontSize: 15, fontWeight: 600 }}>{fmtCurrency(data.endOfHorizon.total, displayCurrency)}</Text>
        </Stack>
      </Group>
    </Box>
  );
