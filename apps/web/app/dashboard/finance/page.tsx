...
function TransactionsView() {
  ...
  const handleCreate = () => {
    if (!form.accountId || !form.categoryId || !form.amount || !form.type || !form.date) return;
    createTx.mutate(form as CreateTransactionDto, { onSuccess: close });
  };
  ...
  return (
    <>
      <Group justify="space-between" mb="md">
        <Text fw={600} size="sm" c="dimmed">All Transactions</Text>
      ...
      {transactions.length === 0 ? (
        <Center py="xl"><Text c="dimmed" size="sm">No transactions yet.</Text></Center>
      ) : (
        <Stack gap="xs">
          {transactions.map((tx) => (
            <TransactionRow
      ...
      <Modal opened={opened} onClose={close} title="New Transaction" centered>
        <Stack gap="sm">
          <Select
            label="Type"
...
function TransactionRow({ tx, currency, onDelete }: { tx: FinanceTransaction; currency?: string; onDelete?: () => void }) {
  ...
  return (
    <Group justify="space-between" p="sm" style={{ borderRadius: 8, background: '#f8fafc', border: '1px solid #E2E8F0' }}>
      <Group gap="sm">
        ...
        <Box>
          <Text size="sm" fw={500}>{tx.description || tx.type}</Text>
