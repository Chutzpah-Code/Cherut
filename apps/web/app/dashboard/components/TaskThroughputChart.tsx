...
export function TaskThroughputChart() {
  const t = useTranslations('dashboard.taskThroughput');
  ...
  return (
    <Box style={{ padding: '24px 28px 26px' }}>
      <Group justify="space-between" align="baseline">
        ...
        <Link href="/dashboard/tasks" style={{ fontSize: 13, fontWeight: 600, color: '#1D4ED8', textDecoration: 'none' }}>{t('tasksLink')}</Link>
      </Group>
      ...
      {isLoading ? (
        ...
        <>
          <Box
            role="img"
            aria-label={t('ariaLabel', { avg: data?.weeklyAvg ?? 0 })}
